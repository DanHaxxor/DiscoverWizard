...
const pp2Options = {
    ...
    service: [
        ...
        { value: 'outgrown-helpdesk', label: 'A help desk tool we\'ve outgrown' },
    ...
    marketing: [
        ...
        { value: 'don\'t-know-where-to-start', label: 'We know we need to do more but don\'t know where to start' },
...
function initCustomSelect(el, initialOptions, onChange) {
    ...
    // Create sheet header for mobile
    const sheetHeader = document.createElement('div');
    sheetHeader.className = 'custom-select__sheet-header';
    sheetHeader.innerHTML = `
        <span class="custom-select__sheet-title">${placeholder.replace('…', '')}</span>
        <button class="custom-select__sheet-close" type="button" aria-label="Close">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
        </button>
    `;
    dropdown.insertBefore(sheetHeader, dropdown.firstChild);

    sheetHeader.querySelector('.custom-select__sheet-close').addEventListener('click', () => close());
    ...
    function open() {
        ...
        if (isMobileView()) {
            ...
            // Force reflow so the transition animates from translateY(100%)
            void dropdown.offsetHeight;
            dropdown.classList.add('sheet-open');
        }
    ...
    function close() {
        ...
        if (dropdown.classList.contains('sheet-open')) {
            dropdown.classList.remove('sheet-open');
            // Return dropdown to its original parent after transition
            const onEnd = () => {
    ...
    document.addEventListener('click', (e) => {
        if (!el.contains(e.target) && !dropdown.contains(e.target)) close();
    });
...
const rootCauseTemplates = {
    ...
    service: {
        ...
        'outgrown-helpdesk': 'The root cause is that your team has outgrown your current help desk. What worked early on now creates friction — missing features force workarounds that slow everyone down.',
    ...
    marketing: {
        ...
        'don\'t-know-where-to-start': 'The root cause is a strategy gap. The intent to grow through marketing is there, but without a structured approach and the right tools, effort gets scattered across too many tactics with too little follow-through.',
